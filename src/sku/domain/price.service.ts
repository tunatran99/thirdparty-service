...
import { MBPriceEntity } from '../infratsructure/entity/mb_price';
...
export class PriceService {
  ...
  async callMobileApp(prices: PriceEntity[] | MBPriceEntity[]) {
  ...
  formatDataForMBApp(prices: PriceEntity[] | MBPriceEntity[], skusNotFound: string[] = []) {
    const success = [],
      error = [];
  ...
  async calcPrice(skusParam?: string[], priceDate?: string, isSPF?: boolean) {
    let eligibleStore = ["1002"]
    ...
    const currTime = now.format('HH:mm:ss');
    ...
    for (const [chunkIndex, skuChunk] of skuChunks.entries()) {
      ...
      const expiredGPCs = rawGPC.filter((i) => {
        if (i.END_DATE && Number.parseInt(i.END_DATE) < Number.parseInt(currDate) && !i.PROCESS_STATUS) {
          return i;
        }
      });
      ...
      for (const [, sku] of skus.entries()) {
        ...
        const expiredPCs = sku.pricechanges.filter((i) => {
          if (i.END_DATE && Number.parseInt(i.END_DATE) < Number.parseInt(currDate)
            && !['PDCM', 'PMOM'].includes(i.TRANS_TYPE) && !i.PROCESS_STATUS) {
            return i;
          }
        });
        ...
        for (const store of sku.stores) {
            ...
            if ('EOP' === pc.TRANS_TYPE) {
              ...
              if (promoPc) {
                ...
                if (notEOP) {
                  ...
                  //   .createQueryBuilder()
                  //   .update()
                  //   .set({ PROCESS_STATUS: notEOP.END_DATE ? 'P' : 'C' })
                  //   .where('SKU = :skuCode', { skuCode: sku.SKU_CODE })
                  ...
                  //   .createQueryBuilder()
                  //   .update()
                  //   .set({ PROCESS_STATUS: pc.END_DATE ? 'P' : 'C' })
                  //   .where('SKU = :skuCode', { skuCode: sku.SKU_CODE })
              ...
              if (mkumkd) {
                ...
                //   .createQueryBuilder()
                //   .update()
                //   .set({ PROCESS_STATUS: mkumkd.END_DATE ? 'P' : 'C' })
                //   .where('SKU = :skuCode', { skuCode: sku.SKU_CODE })
          ...
          let futurePC = filteredPc.find((i) => Number.parseInt(i.START_DATE) > Number.parseInt(currDate));
          //Nếu có pricechange
          if (futurePC) {
            if ('EOP' === futurePC.TRANS_TYPE) {
              const promoPc = filteredPc.find(
                (i) =>
                  i.PRICE_ID !== futurePC.PRICE_ID &&
                  Number.parseInt(i.START_DATE) > Number.parseInt(currDate) &&
                  ['PDC'].includes(i.TRANS_TYPE),
              );
              if (promoPc) {
              ...
              psPrice.futureStartTime = futurePC.START_DATE
                ...
                : null;
              psPrice.futureEndTime = futurePC.END_DATE
                ? moment(`${futurePC.END_DATE}-${futurePC.DAILY_END_TIME}`, 'YYYYMMDD-HHmmss').toDate()
                : null;
            }
          }
          //Nếu không có pricechange nào thì tính theo grouppricechange
          else {
            //Lấy grouppricechane có store, category phù hợp và startdate lớn hơn ngày hiện tại
            const gpc = grouppricechanges.find(
              (i) =>
                i.STORE === psPrice.store &&
                i.CATEGORY === psPrice.category &&
            ...
            if (gpc) {
              psPrice.futureGPCNo = gpc.PRICE_CHANGE_NO;

              const price =
                (Number.parseFloat(psPrice.normalPrice) * (100.0 - Number.parseFloat(gpc.PRICE_CHANGE_TYPE_VALUE))) /
                100;
              psPrice.futurePromoPrice = price.toFixed(2);
              psPrice.futureStartTime = gpc.START_DATE
                ? moment(`${gpc.START_DATE}-${gpc.START_TIME}`, 'YYYYMMDD-HHmmss').toDate()
                : null;
              psPrice.futureEndTime = gpc.END_DATE
                ? moment(`${gpc.END_DATE}-${gpc.END_TIME}`, 'YYYYMMDD-HHmmss').toDate()
                : null;
            }
            else {
          ...
          // }
          // else {
          //   psPrice.oriPromoPrice = null;
          //   // psPrice.startTime = null;
          //   // psPrice.endTime = null;
          //   psPrice.memberMark = false;
          // }
  ...
  async calcMBPrice(skusParam?: string[], priceDate?: string) {
    this.logger.log(`${moment().format('DD/MM/YYYY HH:mm:ss')} - Bắt đầu tính giá MB`);
    ...
    for (const [chunkIndex, skuChunk] of skuChunks.entries()) {
      ...
      for (const [, sku] of skus.entries()) {
        ...
        for (const store of sku.stores) {
          // if (eligibleStore.includes(store)) {
          //Tạo record giá
          const psPrice = readConnection.getRepository(MBPriceEntity).create({
            sku: sku.SKU_CODE,
            store,
            line: sku.LINE_ID,
            division: sku.DIVISION_ID,
    ...
    return { MBPrices: psPrices, errors };
  }
}
